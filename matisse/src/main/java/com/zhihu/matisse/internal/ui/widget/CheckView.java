...
//    private static final int CONTENT_SIZE = 16; // dp
    private static final int DEFAULT_TEXT_SIZE_IN_SP = 12;
...
//    private Paint mShadowPaint;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            width = (int) (SIZE * mDensity);
        }

        if (heightMode == MeasureSpec.EXACTLY) {
            height = heightSize;
        } else {
            height = (int) (SIZE * mDensity);
        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CheckView);
            mCheckTextSize = a.getDimension(R.styleable.CheckView_checkTextSize,
                    DEFAULT_TEXT_SIZE_IN_SP * mDensity);
            mBorderWidth = a.getDimension(R.styleable.CheckView_borderWidth,
                    STROKE_WIDTH * mDensity);
            mCircleRadius = a.getDimension(R.styleable.CheckView_circleRadius,
                    BG_RADIUS * mDensity);
    ...
    public void setCheckedNum(int checkedNum) {
        if (!mCountable) {
            throw new IllegalStateException("CheckView is not countable, call setChecked() " +
                    "instead.");
        }
...
//                (STROKE_RADIUS + STROKE_WIDTH / 2 + SHADOW_WIDTH) * mDensity, mShadowPaint);
        ...
        if (mCountable) {
            if (mCheckedNum != UNCHECKED) {
                initBackgroundPaint();
                canvas.drawCircle(getWidth() / 2, getHeight() / 2, mCircleRadius, mBackgroundPaint);
                initTextPaint();
        ...
        // draw white stroke
        canvas.drawCircle(getWidth() / 2, getHeight() / 2, mCircleRadius - mBorderWidth / 2,
                mStrokePaint);
...
//                            new int[]{Color.parseColor("#00000000"), Color.parseColor
// ("#0D000000"),
...
//    }
    ...
    private void initStrokePaint(){
        ...
        TypedArray ta = getContext().getTheme().obtainStyledAttributes(new int[]{R.attr
                .item_checkCircle_borderColor});
    ...
    private void initUnCheckBackgroundPaint() {
        if (mUnCheckBackgroundPaint == null) {
            ...
            int defaultColor = ResourcesCompat.getColor(
                    getResources(), R.color.zhihu_item_uncheckCircle_backgroundColor,
                    getContext().getTheme());
            int color = ta.getColor(0, defaultColor);
            ta.recycle();
    ...
    private void initTextPaint() {
        if (mTextPaint == null) {
            ...
            mTextPaint.setTextSize(mCheckTextSize);
        }
    }
...
//            int rectPadding = (int) (SIZE * mDensity / 2 - CONTENT_SIZE * mDensity / 2);
            int rectPadding = (int) ((getWidth() - mCircleRadius) / 2);
            mCheckRect = new Rect(rectPadding, rectPadding, getWidth() - rectPadding, getHeight()
                    - rectPadding);
        }
